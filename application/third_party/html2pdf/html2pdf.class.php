...
'.$content.'
	    ...
	    protected function _prepareSubObj()
        {
            ...
            // init
            Pdf::$_subobj->setTestTdInOnePage($this->_testTdInOnepage);
            Pdf::$_subobj->setTestIsImage($this->_testIsImage);
            Pdf::$_subobj->setTestIsDeprecated($this->_testIsDeprecated);
            Pdf::$_subobj->setDefaultFont($this->_defaultFont);
            Pdf::$_subobj->parsingCss->css     = &$this->parsingCss->css;
            Pdf::$_subobj->parsingCss->cssKeys = &$this->parsingCss->cssKeys;

            // clone font from the original PDF
            Pdf::$_subobj->pdf->cloneFontFrom($this->pdf);

            // remove the link to the parent
            Pdf::$_subobj->parsingCss->setPdfParent($pdf);
        }
        ...
        protected function _createSubHTML(&$subHtml, $cellmargin=0)
        {
            ...
            Pdf::$_subobj->pdf->getPage();

            // clone the sub oject
            $subHtml = clone Pdf::$_subobj;
        ...
        protected function _tag_open_THEAD($param)
        {
            ...
            if ($this->_subPart) {
                Pdf::$_tables[$param['num']]['thead']['tr'][0] = Pdf::$_tables[$param['num']]['tr_curr'];
                Pdf::$_tables[$param['num']]['thead']['code'] = array();
                for ($pos=$this->_tempPos; $pos<count($this->parsingHtml->code); $pos++) {
                    ...
                    Pdf::$_tables[$param['num']]['thead']['code'][] = $action;
                    if (strtolower($action['name'])=='thead_sub' && $action['close']) break;
                }
            } else {
	            $level                                  = $this->parsingHtml->getLevel($this->_parsePos);
                $this->_parsePos                        += count($level);
                Pdf::$_tables[$param['num']]['tr_curr'] += count(Pdf::$_tables[$param['num']]['thead']['tr']);
            }
        ...
        protected function _tag_close_THEAD($param)
        {
	        ...
	        if ($this->_subPart) {
                $min                                        = Pdf::$_tables[$param['num']]['thead']['tr'][0];
                $max                                        = Pdf::$_tables[$param['num']]['tr_curr']-1;
                Pdf::$_tables[$param['num']]['thead']['tr'] = range($min, $max);
            }
        ...
        protected function _tag_open_TFOOT($param)
        {
            ...
            if ($this->_subPart) {
                Pdf::$_tables[$param['num']]['tfoot']['tr'][0] = Pdf::$_tables[$param['num']]['tr_curr'];
                Pdf::$_tables[$param['num']]['tfoot']['code'] = array();
                for ($pos=$this->_tempPos; $pos<count($this->parsingHtml->code); $pos++) {
                    $action = $this->parsingHtml->code[$pos];
                    if (strtolower($action['name'])=='tfoot') $action['name'] = 'tfoot_sub';
                    Pdf::$_tables[$param['num']]['tfoot']['code'][] = $action;
                    if (strtolower($action['name'])=='tfoot_sub' && $action['close']) break;
                }
            } else {
	            $level                                  = $this->parsingHtml->getLevel($this->_parsePos);
                $this->_parsePos                        += count($level);
                Pdf::$_tables[$param['num']]['tr_curr'] += count(Pdf::$_tables[$param['num']]['tfoot']['tr']);
            }
        ...
        protected function _tag_close_TFOOT($param)
        {
	        ...
	        if ($this->_subPart) {
                $min                                        = Pdf::$_tables[$param['num']]['tfoot']['tr'][0];
                $max                                        = Pdf::$_tables[$param['num']]['tr_curr']-1;
                Pdf::$_tables[$param['num']]['tfoot']['tr'] = range($min, $max);
            }
        ...
        protected function _tag_open_TABLE($param, $other = 'table')
        {
	        ...
	        if ($this->_subPart) {
                ...
                Pdf::$_tables[$param['num']]                    = array();
                Pdf::$_tables[$param['num']]['border']          = isset($param['border']) ? $this->parsingCss->readBorder($param['border']) : null;
                Pdf::$_tables[$param['num']]['cellpadding']     = $this->parsingCss->ConvertToMM(isset($param['cellpadding']) ? $param['cellpadding'] : '1px');
                Pdf::$_tables[$param['num']]['cellspacing']     = $this->parsingCss->ConvertToMM(isset($param['cellspacing']) ? $param['cellspacing'] : '2px' );
                Pdf::$_tables[$param['num']]['cases']           = array();          // properties of each TR/TD
		        Pdf::$_tables[$param['num']]['corr']            = array();          // link between TR/TD and colspan/rowspan
                Pdf::$_tables[$param['num']]['corr_x']          = 0;                // position in 'cases'
                Pdf::$_tables[$param['num']]['corr_y']          = 0;                // position in 'cases'
                Pdf::$_tables[$param['num']]['td_curr']         = 0;                // current column
                Pdf::$_tables[$param['num']]['tr_curr']         = 0;                // current row
                Pdf::$_tables[$param['num']]['curr_x']          = $this->pdf->getX();
                Pdf::$_tables[$param['num']]['curr_y']          = $this->pdf->getY();
                Pdf::$_tables[$param['num']]['width']           = 0;                // global width
                Pdf::$_tables[$param['num']]['height']          = 0;                // global height
                Pdf::$_tables[$param['num']]['align']           = $alignObject;
                Pdf::$_tables[$param['num']]['marge']           = array();
                Pdf::$_tables[$param['num']]['marge']['t']      = $this->parsingCss->value['padding']['t'] + $this->parsingCss->value['border']['t']['width'] + Pdf::$_tables[$param['num']]['cellspacing'] * 0.5;
                Pdf::$_tables[$param['num']]['marge']['r']      = $this->parsingCss->value['padding']['r'] + $this->parsingCss->value['border']['r']['width'] + Pdf::$_tables[$param['num']]['cellspacing'] * 0.5;
                Pdf::$_tables[$param['num']]['marge']['b']      = $this->parsingCss->value['padding']['b'] + $this->parsingCss->value['border']['b']['width'] + Pdf::$_tables[$param['num']]['cellspacing'] * 0.5;
                Pdf::$_tables[$param['num']]['marge']['l']      = $this->parsingCss->value['padding']['l'] + $this->parsingCss->value['border']['l']['width'] + Pdf::$_tables[$param['num']]['cellspacing'] * 0.5;
                Pdf::$_tables[$param['num']]['page']            = 0;                // number of pages
		        Pdf::$_tables[$param['num']]['new_page']        = true;             // flag : new page for the current TR
                Pdf::$_tables[$param['num']]['style_value']     = null;             // CSS style of the table
                Pdf::$_tables[$param['num']]['thead']           = array();          // properties on the thead
                Pdf::$_tables[$param['num']]['tfoot']           = array();          // properties on the tfoot
		        Pdf::$_tables[$param['num']]['thead']['tr']     = array();          // list of the TRs in the thead
		        Pdf::$_tables[$param['num']]['tfoot']['tr']     = array();          // list of the TRs in the tfoot
                Pdf::$_tables[$param['num']]['thead']['height'] = 0;             // thead height
                Pdf::$_tables[$param['num']]['tfoot']['height'] = 0;             // tfoot height
		        Pdf::$_tables[$param['num']]['thead']['code']   = array();            // HTML content of the thead
		        Pdf::$_tables[$param['num']]['tfoot']['code']   = array();            // HTML content of the tfoot
                Pdf::$_tables[$param['num']]['cols']            = array();              // properties of the COLs

                $this->_saveMargin($this->pdf->getlMargin(), $this->pdf->gettMargin(), $this->pdf->getrMargin());

                $this->parsingCss->value['width']-= Pdf::$_tables[$param['num']]['marge']['l'] + Pdf::$_tables[$param['num']]['marge']['r'];
            } else {
                // we start from the first page and the first page of the table
		        Pdf::$_tables[$param['num']]['page']    = 0;
		        Pdf::$_tables[$param['num']]['td_curr'] = 0;
                Pdf::$_tables[$param['num']]['tr_curr'] = 0;
                Pdf::$_tables[ $param['num']]['td_x']   = Pdf::$_tables[ $param['num']]['marge']['l'] + Pdf::$_tables[ $param['num']]['curr_x'];
                Pdf::$_tables[$param['num']]['td_y']    = Pdf::$_tables[$param['num']]['marge']['t']+Pdf::$_tables[$param['num']]['curr_y'];

		        // draw the borders/background of the first page/part of the table
                $this->_drawRectangle(
	                Pdf::$_tables[$param['num']]['curr_x'],
	                Pdf::$_tables[$param['num']]['curr_y'],
	                Pdf::$_tables[$param['num']]['width'],
                    isset(Pdf::$_tables[$param['num']]['height'][0]) ? Pdf::$_tables[$param['num']]['height'][0] : null,
                    $this->parsingCss->value['border'],
                    $this->parsingCss->value['padding'],
                    0,
                    $this->parsingCss->value['background']
                ...
                Pdf::$_tables[$param['num']]['style_value'] = $this->parsingCss->value;
            }
        ...
        protected function _tag_close_TABLE($param)
        {
	        ...
	        if ($this->_subPart) {
                ...
                $this->_calculateTableCellSize(Pdf::$_tables[$param['num']]['cases'], Pdf::$_tables[$param['num']]['corr']);

                // calculate the height of the thead and the tfoot
                $lst = array('thead', 'tfoot');
		        foreach ($lst as $mode) {
                    Pdf::$_tables[$param['num']][$mode]['height'] = 0;
                    foreach (Pdf::$_tables[$param['num']][$mode]['tr'] as $tr ) {
                        // hauteur de la ligne tr
                        $h = 0;
                        for ($i=0; $i<count(Pdf::$_tables[$param['num']]['cases'][$tr]); $i++)
	                        if ( Pdf::$_tables[$param['num']]['cases'][$tr][$i]['rowspan'] == 1)
                                $h = max($h, Pdf::$_tables[$param['num']]['cases'][$tr][$i]['h']);
                        Pdf::$_tables[$param['num']][$mode]['height'] += $h;
                    }
                }

                // calculate the width of the table
                Pdf::$_tables[$param['num']]['width'] = Pdf::$_tables[$param['num']]['marge']['l'] + Pdf::$_tables[ $param['num']]['marge']['r'];
                if (isset(Pdf::$_tables[$param['num']]['cases'][0])) {
                    foreach (Pdf::$_tables[$param['num']]['cases'][0] as $case) {
                        Pdf::$_tables[$param['num']]['width']+= $case['w'];
                    }
                }
                ...
                $old         = $this->parsingCss->getOldValues();
                $parentWidth = $old['width'] ? $old['width'] : $this->pdf->getW() - $this->pdf->getlMargin() - $this->pdf->getrMargin();
                $x           = Pdf::$_tables[$param['num']]['curr_x'];
                $w           = Pdf::$_tables[$param['num']]['width'];
                if ($parentWidth>$w) {
                    if ( Pdf::$_tables[$param['num']]['align'] == 'center')
                        $x = $x + ($parentWidth-$w)*0.5;
                    else if ( Pdf::$_tables[$param['num']]['align'] == 'right')
                        $x = $x + $parentWidth-$w;

                    Pdf::$_tables[$param['num']]['curr_x'] = $x;
                }

                // calculate the height of the table
                Pdf::$_tables[$param['num']]['height'] = array();

                // minimum of the height because of margins, and of the thead and tfoot height
		        $h0 = Pdf::$_tables[$param['num']]['marge']['t'] + Pdf::$_tables[$param['num']]['marge']['b'];
                $h0 += Pdf::$_tables[$param['num']]['thead']['height'] + Pdf::$_tables[$param['num']]['tfoot']['height'];

                // max height of the page
		        $max = $this->pdf->getH() - $this->pdf->getbMargin();

                // current position on the page
                $y = Pdf::$_tables[$param['num'] ]['curr_y'];
                $height = $h0;

                // we get the height of each line
                for ($k=0; $k<count(Pdf::$_tables[$param['num']]['cases']); $k++) {

                    // if it is a TR of the thead or of the tfoot => skip
	                if (in_array($k, Pdf::$_tables[$param['num']]['thead']['tr'])) continue;
                    if (in_array($k, Pdf::$_tables[$param['num']]['tfoot']['tr'])) continue;

	                // height of the line
                    $th = 0;
                    $h = 0;
	                for ($i=0; $i<count(Pdf::$_tables[$param['num']]['cases'][$k ] ); $i++) {
                        $h = max($h, Pdf::$_tables[$param['num']]['cases'][$k][$i]['h']);

		                if ( Pdf::$_tables[$param['num']]['cases'][$k][$i]['rowspan'] == 1)
                            $th = max($th, Pdf::$_tables[$param['num']]['cases'][$k][$i]['h']);
                    }

                    // if the row does not fit on the page => new page
                    if ($y+$h+$height>$max) {
                        if ($height==$h0) $height = null;
                        Pdf::$_tables[$param['num']]['height'][] = $height;
                        $height = $h0;
                        $y = $this->_margeTop;
                    }
                    $height += $th;
                ...
                if ($height!=$h0 || $k==0) Pdf::$_tables[ $param['num']]['height'][] = $height;
            } else {
                // if we have tfoor, draw it
                if (count(Pdf::$_tables[$param['num']]['tfoot']['code'])) {
                    $tmpTR        = Pdf::$_tables[$param['num']]['tr_curr'];
                    $tmpTD        = Pdf::$_tables[$param['num']]['td_curr'];
	                $oldParsePos  = $this->_parsePos;
                    $oldParseCode = $this->parsingHtml->code;

	                Pdf::$_tables[$param['num']]['tr_curr'] = Pdf::$_tables[$param['num']]['tfoot']['tr'][0];
                    Pdf::$_tables[$param['num']]['td_curr'] = 0;
                    $this->_parsePos                        = 0;
                    $this->parsingHtml->code                = Pdf::$_tables[$param['num']]['tfoot']['code'];
	                ...
	                $this->_parsePos                        =     $oldParsePos;
                    $this->parsingHtml->code                = $oldParseCode;
                    Pdf::$_tables[$param['num']]['tr_curr'] = $tmpTR;
                    Pdf::$_tables[$param['num']]['td_curr'] = $tmpTD;
                }

                // get the positions of the end of the table
                $x = Pdf::$_tables[$param['num']]['curr_x'] + Pdf::$_tables[$param['num']]['width'];
                if ( count( Pdf::$_tables[$param['num']]['height']) > 1)
                    $y = $this->_margeTop + Pdf::$_tables[$param['num']]['height'][ count(Pdf::$_tables[ $param['num']]['height']) - 1];
                else if ( count(Pdf::$_tables[$param['num']]['height']) == 1)
                    $y = Pdf::$_tables[$param['num']]['curr_y']+Pdf::$_tables[$param['num']]['height'][ count(Pdf::$_tables[$param['num']]['height']) - 1];
                else
                    $y = Pdf::$_tables[$param['num']]['curr_y'];
        ...
        protected function _tag_open_COL($param ) {
            ...
            for ($k=0; $k<$span; $k++)
                Pdf::$_tables[$param['num']]['cols'][] = $param;
        }
        ...
        protected function _tag_open_TR($param, $other = 'tr')
        {
	        ...
	        if (!$this->_subPart) {
		        ...
		        // Y after the row
                $ty=null;
                for ($ii=0; $ii<count(Pdf::$_tables[$param['num']]['cases'][ Pdf::$_tables[$param['num']]['tr_curr'] - 1]); $ii++) {
                    $ty = max($ty, Pdf::$_tables[$param['num']]['cases'][ Pdf::$_tables[$param['num']]['tr_curr'] - 1][$ii]['h']);
                }

                // height of the tfoot
                $hfoot = Pdf::$_tables[$param['num']]['tfoot']['height'];

                // if the line does not fit on the page => new page
                if (!$this->_isInTfoot && Pdf::$_tables[$param['num']]['td_y'] + Pdf::$_tables[$param['num']]['marge']['b'] + $ty +$hfoot> $this->pdf->getH() - $this->pdf->getbMargin()) {
                    ...
                    // new page
                    Pdf::$_tables[$param['num']]['new_page'] = true;
                    $this->_setNewPage();

                    // new position
                    Pdf::$_tables[$param['num']]['page']++;
	                Pdf::$_tables[$param['num']]['curr_y'] = $this->pdf->getY();
                    Pdf::$_tables[$param['num']]['td_y']   = Pdf::$_tables[$param['num']]['curr_y']+Pdf::$_tables[$param['num']]['marge']['t'];

	                // if we have the height of the tbale on the page => draw borders and background
                    if (isset(Pdf::$_tables[$param['num']]['height'][Pdf::$_tables[$param['num']]['page']])) {
                        $old                     = $this->parsingCss->value;
                        $this->parsingCss->value = Pdf::$_tables[$param['num']]['style_value'];

	                    $this->_drawRectangle(
                            Pdf::$_tables[$param['num']]['curr_x'],
                            Pdf::$_tables[$param['num']]['curr_y'],
		                    Pdf::$_tables[$param['num']]['width'],
                            Pdf::$_tables[$param['num']]['height'][Pdf::$_tables[$param['num']]['page']],
		                    $this->parsingCss->value['border'],
                            $this->parsingCss->value['padding'],
                            Pdf::$_tables[$param['num']]['cellspacing']*0.5,
                            $this->parsingCss->value['background']
                        );
		        ...
		        if ( Pdf::$_tables[$param['num']]['new_page'] && count(Pdf::$_tables[$param['num']]['thead']['code'])) {
                    Pdf::$_tables[$param['num']]['new_page'] = false;
                    $tmpTR                                   = Pdf::$_tables[$param['num']]['tr_curr'];
                    $tmpTD                                   = Pdf::$_tables[$param['num']]['td_curr'];
			        $oldParsePos                             = $this->_parsePos;
                    $oldParseCode                            = $this->parsingHtml->code;

			        Pdf::$_tables[$param['num']]['tr_curr'] = Pdf::$_tables[$param['num']]['thead']['tr'][0];
                    Pdf::$_tables[$param['num']]['td_curr'] = 0;
                    $this->_parsePos                        = 0;
                    $this->parsingHtml->code                = Pdf::$_tables[$param['num']]['thead']['code'];
                ...
                // prepare it
		        Pdf::$_tables[$param['num']]['cases'][ Pdf::$_tables[$param['num']]['tr_curr'] - 1] = array();
                if (!isset(Pdf::$_tables[$param['num']]['corr'][Pdf::$_tables[$param['num']]['corr_y']]))
                    Pdf::$_tables[$param['num']]['corr'][Pdf::$_tables[$param['num']]['corr_y']] = array();

                Pdf::$_tables[$param['num']]['corr_x'] =0;
                while(isset(Pdf::$_tables[$param['num']]['corr'][Pdf::$_tables[$param['num']]['corr_y']][Pdf::$_tables[$param['num']]['corr_x']]))
                    Pdf::$_tables[$param['num']]['corr_x']++;
            }
        ...
        protected function _tag_close_TR($param)
        {
	        ...
	        if (!$this->_subPart) {
                ...
                $ty=null;
                for ($ii =0; $ii < count(Pdf::$_tables[$param['num']]['cases'][ Pdf::$_tables[$param['num']]['tr_curr'] - 1]); $ii++) {
                    if ( Pdf::$_tables[$param['num']]['cases'][ Pdf::$_tables[$param['num']]['tr_curr'] - 1][$ii]['rowspan'] == 1) {
                        $ty = Pdf::$_tables[$param['num']]['cases'][ Pdf::$_tables[$param['num']]['tr_curr'] - 1][$ii]['h'];
                    }
                }

                // new position
                Pdf::$_tables[$param['num']]['td_x']     = Pdf::$_tables[$param['num']]['curr_x'] + Pdf::$_tables[$param['num']]['marge']['l'];
                Pdf::$_tables[$param['num']]['td_y']     += $ty;
                Pdf::$_tables[$param['num']]['new_page'] = false;
            } else {
                Pdf::$_tables[$param['num']]['corr_y']++;
            }
        ...
        protected function _tag_open_TD($param, $other = 'td') {
            ...
            $param['cellpadding'] = Pdf::$_tables[$param['num']]['cellpadding'].'mm';
            $param['cellspacing'] = Pdf::$_tables[$param['num']]['cellspacing'].'mm';
            ...
            $x = Pdf::$_tables[$param['num']]['td_curr'];
            $y = Pdf::$_tables[$param['num']]['tr_curr']-1;
            ...
            if (in_array($other, array('td', 'th'))) {
                // id of the column
	            $numCol = isset(Pdf::$_tables[$param['num']]['cases'][$y][$x]['Xr']) ? Pdf::$_tables[$param['num']]['cases'][$y][$x]['Xr'] : Pdf::$_tables[$param['num']]['corr_x'];

                // we get the properties of the COL tag, if exist
                if (isset(Pdf::$_tables[$param['num']]['cols'][$numCol])) {

                    $colParam = Pdf::$_tables[$param['num']]['cols'][$numCol];

                    // for colspans => we get all the neede widths
	                $colParam['style']['width'] = array();
                    for ($k=0; $k<$colspan; $k++) {
                        if (isset( Pdf::$_tables[$param['num']]['cols'][ $numCol + $k]['style']['width'])) {
                            $colParam['style']['width'][] = Pdf::$_tables[$param['num']]['cols'][$numCol+$k]['style']['width'];
                        }
                    }
            ...
            if (in_array($other, array('td', 'th'))) {
                ...
                if (Pdf::$_tables[$param['num']]['border']) {
                    $legacy['border'] = array();
	                $legacy['border']['l'] = Pdf::$_tables[$param['num']]['border'];
	                $legacy['border']['t'] = Pdf::$_tables[$param['num']]['border'];
	                $legacy['border']['r'] = Pdf::$_tables[$param['num']]['border'];
                    $legacy['border']['b'] = Pdf::$_tables[$param['num']]['border'];
                }
            }
            ...
            if ($collapse) {
	            if (!$this->_subPart) {
                    if (
	                    ( Pdf::$_tables[ $param['num']]['tr_curr'] > 1 && !Pdf::$_tables[$param['num']]['new_page']) ||
	                    (!$this->_isInThead && count(Pdf::$_tables[$param['num']]['thead']['code']))
                    ) {
                        $this->parsingCss->value['border']['t'] = $this->parsingCss->readBorder('none');
                    }
                }

                if (Pdf::$_tables[$param['num']]['td_curr']>0) {
            ...
            $marge = array();
            $marge['t'] = $this->parsingCss->value['padding']['t']+0.5*Pdf::$_tables[$param['num']]['cellspacing'] + $this->parsingCss->value['border']['t']['width'];
            $marge['r'] = $this->parsingCss->value['padding']['r']+0.5*Pdf::$_tables[$param['num']]['cellspacing'] + $this->parsingCss->value['border']['r']['width'];
            $marge['b'] = $this->parsingCss->value['padding']['b']+0.5*Pdf::$_tables[$param['num']]['cellspacing'] + $this->parsingCss->value['border']['b']['width'];
            $marge['l'] = $this->parsingCss->value['padding']['l']+0.5*Pdf::$_tables[$param['num']]['cellspacing']+$this->parsingCss->value['border']['l']['width'];

	        // if we are in a sub HTML
            if ($this->_subPart) {
                // new position in the table
                Pdf::$_tables[$param['num']]['td_curr']++;
                Pdf::$_tables[ $param['num']]['cases'][$y][$x]            = array();
                Pdf::$_tables[ $param['num']]['cases'][$y][$x]['w']       = 0;
                Pdf::$_tables[ $param['num']]['cases'][$y][$x]['h']       = 0;
                Pdf::$_tables[$param['num'] ]['cases'][$y][$x]['dw']      = 0;
                Pdf::$_tables[$param['num']]['cases'][$y ][$x]['colspan'] = $colspan;
                Pdf::$_tables[$param['num']]['cases'][$y][$x]['rowspan']  = $rowspan;
                Pdf::$_tables[$param['num']]['cases'][$y][$x]['Xr']       = Pdf::$_tables[$param['num']]['corr_x'];
                Pdf::$_tables[$param['num']]['cases'][$y][$x]['Yr']       = Pdf::$_tables[$param['num']]['corr_y'];

                // prepare the mapping for rowspan and colspan
                for ($j=0; $j<$rowspan; $j++) {
	                for ($i=0; $i < $colspan; $i++) {
                        Pdf::$_tables[$param['num']]['corr']
                            [Pdf::$_tables[$param['num']]['corr_y']+$j]
                            [ Pdf::$_tables[$param['num']]['corr_x'] + $i] = ( $i + $j > 0) ? '' : array($x,$y,$colspan,$rowspan);
	                }
                }
	            Pdf::$_tables[$param['num'] ]['corr_x'] += $colspan;
                while (isset(Pdf::$_tables[$param['num']]['corr'][Pdf::$_tables[$param['num']]['corr_y']][Pdf::$_tables[$param['num']]['corr_x']])) {
                    Pdf::$_tables[$param['num']]['corr_x']++;
                }
                ...
                // new position in the table
                Pdf::$_tables[$param['num']]['td_curr']++;
                Pdf::$_tables[$param['num']]['td_x']+= Pdf::$_tables[$param['num']]['cases'][$y][$x]['dw'];

                // borders and background of the TD
                $this->_drawRectangle(
	                Pdf::$_tables[$param['num']]['td_x'],
	                Pdf::$_tables[$param['num']]['td_y'],
                    Pdf::$_tables[$param['num']]['cases'][$y][$x]['w'],
                    Pdf::$_tables[$param['num']]['cases'][$y][$x]['h'],
	                $this->parsingCss->value['border'],
                    $this->parsingCss->value['padding'],
                    Pdf::$_tables[$param['num']]['cellspacing']*0.5,
	                $this->parsingCss->value['background']
                ...
                $this->parsingCss->value['width'] = Pdf::$_tables[$param['num']]['cases'][$y][$x]['w'] - $marge['l'] - $marge['r'];

                // marges = size of the TD
                $mL = Pdf::$_tables[$param['num']]['td_x']+$marge['l'];
                $mR = $this->pdf->getW() - $mL - $this->parsingCss->value['width'];
	            $this->_saveMargin($mL, 0, $mR);

                // position of the content, from vertical-align
                $hCorr = Pdf::$_tables[$param['num']]['cases'][$y][$x]['h'];
                $hReel = Pdf::$_tables[$param['num']]['cases'][$y][$x]['real_h'];
                switch($this->parsingCss->value['vertical-align'])
                {
                    case 'bottom':
                        $yCorr = $hCorr-$hReel;
                ...
                $x = Pdf::$_tables[$param['num']]['td_x']+$marge['l'];
                $y = Pdf::$_tables[$param['num']]['td_y']+$marge['t']+$yCorr;
                $this->pdf->setXY($x, $y);
                $this->_setNewPositionForNewLine();
            }
        ...
        protected function _tag_close_TD($param)
        {
            ...
            if ($this->_subPart) {
                ...
                Pdf::$_tables[$param['num']]['cases'][ Pdf::$_tables[$param['num']]['tr_curr'] - 1][ Pdf::$_tables[$param['num']]['td_curr'] - 1]['w'] = max(array($w0, $w2));
                Pdf::$_tables[$param['num']]['cases'][Pdf::$_tables[$param['num']]['tr_curr']-1][ Pdf::$_tables[$param['num']]['td_curr'] - 1]['h']    = max(array($h0, $h2));

	            // real position of the content
                Pdf::$_tables[$param['num'] ]['cases'][ Pdf::$_tables[$param['num'] ]['tr_curr'] - 1][ Pdf::$_tables[$param['num'] ]['td_curr'] - 1]['real_w'] = $w0;
                Pdf::$_tables[$param['num']]['cases'][Pdf::$_tables[$param['num']]['tr_curr']-1][Pdf::$_tables[$param['num']]['td_curr']-1]['real_h']          = $h0;
	            ...
	            Pdf::$_tables[$param['num']]['td_x'] += Pdf::$_tables[$param['num']]['cases'][ Pdf::$_tables[$param['num']]['tr_curr'] - 1][ Pdf::$_tables[$param['num']]['td_curr'] - 1]['w'];
            }
