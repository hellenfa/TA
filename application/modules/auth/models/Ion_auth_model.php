...
//    var $tbl_unit_ugm = 'tbl_unit_ugm';
    ...
    public function get_users($start, $count)
    {
        $res = $this->db->select('*')->from($this->table)->limit($count, ($start * $count))->get();
    ...
    public function is_member_group($user_id, $group_id)
    {
        $where = array('user_id' => $user_id, 'group_id' => $group_id);

        $data = $this->db->select('user_id')->from('users_groups')->where($where)->get();
    ...
    public function is($grup)
    {
        switch ($grup) {
            case "keuangan":
                $wh = array('user_id' => $this->ion_auth->user()->row()->id, 'group_id' => '2');
                break;
            case "pengelola_surat":
                $wh = array('user_id' => $this->ion_auth->user()->row()->id, 'group_id' => '3');
                break;
            default:
                break;
        }

        $data = $this->db->select('*')->from('users_groups')->where($wh)->get();
    ...
    {
        $data = $this->db->where('username', $username)->count_all_results($this->table);
    ...
    public function upload_profile_picture($id_user,$data)
    {
        $this->db->where('id',$id_user)->set($data)->update($this->table);
